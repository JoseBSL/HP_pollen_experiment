...
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="BROL 50%"] <- "Wild cabagge"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="BRRA 50%"] <- "Pak choi"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="CAAN 50%"] <- "Capsicum"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="ERSA 50%"] <- "Rocket" #Eruca sativa seems to be a synonym
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="IPAQ 50%"] <- "Water morning glory"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="IPPU 50%"] <- "Morning glory"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="PEIN 50%"] <- "Petunia"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="SIAL 50%"] <- "White mustard"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="SOLY 50%"] <- "Tomato"
ippu_seed_set_final$Treatment[ippu_seed_set_final$Treatment=="SOME 50%"] <- "Eggplant"
...
ippu_seed_set_brassicaceae <- filter(ippu_seed_set_final, Family %in% c("Brassicaceae"))
ippu_seed_set_convolvulaceae <- filter(ippu_seed_set_final, Family %in% c("Convolvulaceae"))
ippu_seed_set_solanaceae <- filter(ippu_seed_set_final, Family %in% c("Solanaceae"))
ippu_seed_set_final$Family[is.na(ippu_seed_set_final$Family)] <- "Solanum lycopersicum"

ippu_seed_set_cross=ippu_seed_set_cross[,-c(5,6)]
ippu_seed_set_cross$Family <- "other"
ippu_seed_set_self=ippu_seed_set_self[,-c(5,6)]
ippu_seed_set_self$Family <- "other"
ippu_seed_set_control=ippu_seed_set_control[,-c(5,6)]
ippu_seed_set_control$Family <- "other"
ippu_seed_set_flower=ippu_seed_set_flower[,-c(5,6)]
ippu_seed_set_flower$Family <- "other"
