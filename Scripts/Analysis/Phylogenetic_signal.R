...
traits_all_no_sol <- traits_all_no_sol[-c(3,7,9,10),-c(3,7,9,10)]
...
rownames(mean_ovules) <- traits_all_no_sol[[3]]
mean_ovules <- as.matrix((mean_ovules))[,1]
...
#Anthers
anthers <-as.data.frame(traits_all_no_sol[,c("anthers")])
rownames(anthers) <- tree_no_sol[[3]]
anthers <- as.matrix((anthers))[,1]
anthers <- phylosig(tree=tree_no_sol,x=anthers,method="lambda",test=TRUE)
...
#Stigma_surface
stigma_surface <-as.data.frame(traits_all_no_sol[,c("stigma_surface")])
rownames(stigma_surface) <- tree_no_sol[[3]]
stigma_surface <- as.matrix((stigma_surface))[,1]
stigma_surface <- phylosig(tree=tree_no_sol,x=stigma_surface,method="lambda",test=TRUE)
#$`lambda`[1] 0.7292784
