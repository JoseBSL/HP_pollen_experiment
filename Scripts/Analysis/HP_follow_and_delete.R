...
#glm without data transfromation
model1_glm <-  MASS::glm.nb(formula = Seed.production ~ Treatment, data = pein_seed_set_final)
summary(model1_glm)


pein <- summ(model1_glm, model.info=FALSE, model.fit=FALSE)
pein <- as.data.frame(pein$coeftable)
rownames(pein) <- gsub("Treatment", "", rownames(pein))
rownames(pein) <- paste("M1 Petunia integrifolia", rownames(pein), sep=" - ")


model1_glm <- pscl::hurdle(formula = Seed.production ~ Treatment, data = pein_seed_set_final, dist = "negbin")
summary(model1_glm)
plot(model1_glm)

simulationOutput <- simulateResiduals(fittedModel = model1_glm, plot = T)


library(pscl)

model.zi = zeroinfl(Seed.production ~ Treatment,
                    data = pein_seed_set_final,
                    dist = "poisson")

qqnorm(residuals(model.zi, type="response"))
...
library(DHARMa)
testDispersion(m2) 
hist(pein_seed_set_final$Seed.production)


#ER BUENO, SEGUIR MAÑANA Y ARREGLAR
glm_1 <- glm(Seed.production ~ Treatment, data = pein_seed_set_final, family = "quasipoisson")
qqnorm(statmod::qresid(m2)); qqline(statmod::qresid(m2))
summary(glm_1)

glm_1 <- glm(Seed.production ~ Treatment, data = some_seed_set_final, family = "quasipoisson")


summary(m2 <- glm.nb(Seed.production ~ Treatment , data = pein_seed_set_final))
plot(m2)
plot(density(resid(m, type='response')))
lines(density(resid(m2, type='response')), col='red')
scatter.smooth(predict(m2, type='response'), rstandard(m2, type='deviance'), col='gray')
scatter.smooth(sqrt(predict(m2, type='response')), statmod::qresid(m2), col='gray')
qqnorm(statmod::qresid(m2)); qqline(statmod::qresid(m2))
...
#glm without data transfromation
model2_glm <- glm(formula = Seed.production ~ Treatment, family = poisson, data = soly_seed_set_final)
summary(model2_glm)
pein <- summ(model1_glm, model.info=FALSE, model.fit=FALSE)
pein <- as.data.frame(pein$coeftable)
rownames(pein) <- gsub("Treatment", "", rownames(pein))
