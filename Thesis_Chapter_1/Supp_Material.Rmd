...
__Table S4.__ Total number of pollen grains per stigma.

__Table S5.__ Species x species matrix with the significance of effect “yes” or “no” of the different donors on the seed set of the different recipient species.

__Table S6.__ Estimates, standard error, t-value and P-value of the effect of the different 9 donors on each recipient species.

__Table S7.__ Number of seeds produced with 100% foreign pollen treatments for the different recipient species. 
...
dat <- dat[,-5]
colnames(dat) <- c("Recipient", "Donor", "Recipient pollen grains", "Donor pollen grains")
  
kable(dat, "latex", longtable = F, caption = "\\textbf{Table S4.} Average pollen grains per stigma for both recipient and donor species for 20 different treatments (3 replicates per treatment)", escape = FALSE,booktabs = T,linesep = "\\addlinespace") 
...
kable(seed_100, "latex", longtable = F,digits=2,booktabs = T, linesep = "\\addlinespace", caption= "\\textbf{Table S7.} Number of seeds produced with 100\\% foreign pollen (N=900). From 900 pollination events we found seed production in just 13 specific cases and 0 seed production in the other 887 pollination events. In this table we show just the treatments that lead to seed production.") %>% kable_styling(font_size = 10,position="left",full_width=F,latex_options = c("striped","scale_down")) %>% row_spec(0,bold=TRUE)  %>%
