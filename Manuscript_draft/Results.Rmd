...
Here I perform Mantel test and Procustes test between the traits and heterospecific pollen effect. For both Mantel and procustes we were not able to find evidence of correlation between matrices. At the moment we are using all the traits without filtering the correlated traits. With Bioenv we can know what are the traits that gives a best fit of correlation between matrices. The best model is with pollen ovule ratio, stigma width and style width.
...
Interestignly for Brassicaceae species selfing rates matches pretty well the output of heterospecific pollen effect. Therefore, seems that selfing rates in Brassicaceae species could be a good predictor of foreign pollen effect. When we consider the relatedness of the donors, for instance crosses between families 
...
axis.text.x=element_blank(),axis.ticks.x=element_blank(),legend.position = c(0.7, 0.8))+
