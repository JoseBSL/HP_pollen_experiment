...
p11<- ggplot(brol_effect_size, aes(Family,Cohen_d, size=10)) + theme_bw(base_size=10)+theme(legend.direction="horizontal",legend.title = element_blank(),axis.text.y = element_blank())+ geom_point(size=2,aes(color=factor(Family))) +geom_errorbar(size=0.6,alpha=0.5, aes(x = Family, ymin = Lower, ymax = Upper,color=factor(Family)))+scale_color_manual("Family",values=c("#D55E00","#009E73","#E69F00","#0072B2"))
 #grid.arrange(p1, p5, p3, p4, nrow=2,ncol=2,top="Solanaceae")
mylegend<-g_legend(p11)
