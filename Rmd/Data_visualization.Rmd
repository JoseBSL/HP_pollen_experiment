...
```{r,message=FALSE, warning=FALSE, echo=FALSE, cache=FALSE, fig.height=3.5, fig.width=5,fig.show='asis',out.width='55%'}
#Preparing seed set plots, starting with Petunia
...
a <- ggplot(pein_seed_set_final, aes(x = factor(Treatment, levels=unique(Treatment)), y = Seed.production)) +   geom_boxplot()+
  ...
  theme(plot.title = element_text(hjust = 0.5),plot.margin=unit(c(1,1,-0.5,1), "cm")) + geom_jitter(width = 0.2) + stat_summary(fun.y=mean, geom="point", shape="*", size=5, colour="black") +theme(legend.position="none")
...
c <- ggplot(soly_seed_set_final, aes(x = factor(Treatment, levels=unique(Treatment)), y = Seed.production)) +   geom_boxplot()+
  labs(title="Solanum lycopersicum",x="", y = "Seeds")+aes(fill=Family)+theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  theme(plot.title = element_text(hjust = 0.5), plot.margin=unit(c(-0.5,1,1,1), "cm")) + geom_jitter(width = 0.2) + stat_summary(fun.y=mean, geom="point", shape="*", size=5, colour="black") +theme(legend.position="none")
...
grid.arrange(lg[[1]], lg[[2]], lg[[3]], lg[[4]], nrow=2,ncol=2)
